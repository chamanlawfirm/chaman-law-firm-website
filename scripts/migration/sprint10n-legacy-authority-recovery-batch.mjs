...
const DRAFT_PREFIX = "drafts.chamanlawfirm.sprint10n.";
const PUBLIC_PREFIX = "chamanlawfirm-sprint10n-";
const SOURCE_NAME = "legacy-wordpress-backup-2026-06-28-sprint10n";
...
const extractedImageRoot = path.join(process.cwd(), ".tmp", "chaman-sprint10n-images");

const top1000Path = "docs/SPRINT-9M-TOP-1000-LEGACY-BLOG-SELECTION.csv";
const nextConfigPath = "next.config.mjs";
const outDir = "docs/sprint10n";
const selectionCsvPath = "docs/SPRINT-10N-100-BLOG-RECOVERY-SELECTION.csv";
const imageMapPath = "docs/SPRINT-10N-IMAGE-RECOVERY-AND-REFRESH-MAP.csv";
const hiddenReviewPath = "docs/SPRINT-10N-HIDDEN-RISK-CLEANUP.csv";
const resultJsonPath = "docs/sprint10n/sprint10n-restoration-result.json";
...
async function main() {
  ...
  const [topRows, rankRows, nextConfigText, sprint10iResultText, sprint10jResultText, sprint10kResultText, sprint10lResultText] = await Promise.all([
    readCsv(top1000Path),
  ...
  for (const [index, item] of selected.entries()) {
    ...
    selectionRows.push({
      ...
      "current production status": approvedSlugs.has(slug) ? "already public" : "selected for Sprint 10N recovery",
    ...
    imageRows.push({
      ...
      "repeated image risk": "not detected in Sprint 10N recovered image evidence",
...
main().catch((error) => {
  console.error(`Sprint 10N pipeline failed: ${error?.message || String(error)}`);
  process.exit(1);
});
