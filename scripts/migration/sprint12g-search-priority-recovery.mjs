...
const APPROVAL_LIMIT = Number(process.env.SPRINT_12G_APPROVAL_LIMIT || 8);
const MIN_BODY_CHARS = 1400;
const MIN_BODY_BLOCKS = 8;
const EDITORIAL_APPROVAL_SLUGS = new Set(
  String(process.env.SPRINT_12G_APPROVAL_SLUGS || "")
    .split(",")
    .map((slug) => slug.trim())
    .filter(Boolean)
);
...
const paths = {
  ...
  sprint12dPriority: path.join("docs", "SPRINT-12D-600-REPAIRED-RECORD-PRIORITY.csv"),
  sprint12eManual: path.join("docs", "SPRINT-12E-HIGH-VALUE-MANUAL-CLEARANCE.csv"),
  sprint12fDisposition: path.join("docs", "SPRINT-12F-50-ITEM-FINAL-DISPOSITION.csv"),
  nextConfig: "next.config.mjs",
  gscImport: path.join("docs", "SPRINT-12G-GSC-EVIDENCE-IMPORT.csv"),
  gscMissing: path.join("docs", "SPRINT-12G-GSC-EVIDENCE-MISSING.md"),
  bingImport: path.join("docs", "SPRINT-12G-BING-EVIDENCE-IMPORT.csv"),
  bingMissing: path.join("docs", "SPRINT-12G-BING-EVIDENCE-MISSING.md"),
  buildStability: path.join("docs", "SPRINT-12G-LOCAL-BUILD-STABILITY-CHECK.md"),
  freshEvidence: path.join("docs", "SPRINT-12G-FRESH-SEARCH-EVIDENCE-STATUS.md"),
  serviceMonitoring: path.join("docs", "SPRINT-12G-SERVICE-PAGE-MONITORING.csv"),
  redirectMonitoring: path.join("docs", "SPRINT-12G-EXISTING-REDIRECT-MONITORING.csv"),
  searchQueue: path.join("docs", "SPRINT-12G-SEARCH-EVIDENCE-PRIORITY-QUEUE.csv"),
  blogRecovery: path.join("docs", "SPRINT-12G-IMAGE-FIRST-ARTICLE-RECOVERY.csv"),
  imageCtaMetadata: path.join("docs", "SPRINT-12G-IMAGE-CTA-METADATA-REPAIR.csv"),
  legalEditorial: path.join("docs", "SPRINT-12G-LAWYER-SAFE-CLEARANCE.csv"),
  approvalBatch: path.join("docs", "SPRINT-12G-BLOG-PUBLICATION-BATCH.csv"),
  staticRecovery: path.join("docs", "SPRINT-12G-STATIC-SERVICE-RECOVERY.csv"),
  redirectRescue: path.join("docs", "SPRINT-12G-HIGH-VALUE-404-RECOVERY.csv"),
  gscValidation: path.join("docs", "SPRINT-12G-GSC-LIVE-VALIDATION.md"),
  bingValidation: path.join("docs", "SPRINT-12G-BING-LIVE-VALIDATION.md"),
  ga4Validation: path.join("docs", "SPRINT-12G-GA4-LIVE-VALIDATION.md"),
  vercelDomain: path.join("docs", "SPRINT-12G-VERCEL-DOMAIN-VALIDATION.md"),
  sanityCapacity: path.join("docs", "SPRINT-12G-SANITY-CAPACITY-VALIDATION.md"),
  lawzana: path.join("docs", "SPRINT-12G-LAWZANA-STATUS.md"),
  seoGeo: path.join("docs", "SPRINT-12G-SEO-AEO-GEO-PUBLIC-QUALITY.csv"),
  indexingPack: path.join("docs", "SPRINT-12G-GSC-BING-INDEXING-PACK.md"),
  report: path.join("docs", "SPRINT-12G-REPORT.md"),
  result: path.join("docs", "SPRINT-12G-RESULT.json")
};
...
async function fetchManual(url) {
  try {
    const response = await fetch(url, {
      redirect: "manual",
      headers: {
        "user-agent": "ChamanLawFirm-Sprint12G-QA/1.0"
      }
    });
    return {
      ok: true,
...
function docCompleteness(doc, row, configuredRedirects) {
  ...
  if (EDITORIAL_APPROVAL_SLUGS.size && !EDITORIAL_APPROVAL_SLUGS.has(slug)) {
    blockers.push("not selected for Sprint 12G controlled editorial approval");
  }
  if (/\b(with|under|and|of|to)$/i.test(row.title || doc?.title || "")) blockers.push("title appears truncated or incomplete");
...
function publicSafePostId(slug) {
  return `chamanlawfirm-sprint12g-${slug}`;
}

function patchablePublicDoc(hiddenDoc, row, assessment) {
  const slug = assessment.slug;
  const title = hiddenDoc.title || row.title || titleFromSlug(slug);
  const metaTitle = assessment.seoTitle || `${title} | Chaman Law Firm`;
...
function currentRouteSetFromPracticeAreas() {
  ...
  // The data file interleaves practice-area and service-page slugs. For Sprint 12G,
...
async function main() {
  ...
  const baseBlogRows = readCsv(paths.sprint11tBlog).slice(0, 166);
  const manualApprovalRows = [...EDITORIAL_APPROVAL_SLUGS].map((slug, index) => ({
    priority: `12G-${index + 1}`,
    "old URL": absoluteUrl(`/${slug}/`),
    slug,
    title: titleFromSlug(slug),
    category: "Sprint 12G explicit low-risk hidden Sanity candidate"
  }));
  const seenBlogSlugs = new Set();
  const sprint11tBlogRows = [...manualApprovalRows, ...baseBlogRows].filter((row) => {
  ...
  writeMd(
    ...
    `# Sprint 12G Local Build Stability Check
...
- Stability decision: safe to continue Sprint 12G, while continuing to monitor Sanity network fetch warnings during later builds.
...
`
  ...
  writeMd(
    paths.freshEvidence,
    `# Sprint 12G Fresh GSC/Bing Evidence Status
...
`
  ...
  for (const row of sprint11tBlogRows) {
    ...
    const assessment = docCompleteness(hiddenDoc, row, configuredRedirects);
    ...
    const reason = alreadyPublic
      ? "public canonical article already exists"
      : approvalReady
  ...
  const selected = hiddenReadyCandidates.slice(0, APPROVAL_LIMIT);
  const sanityChanges = [];
  if (APPLY) {
    for (const candidate of selected) {
      const operations = await approveCandidate(candidate.row, candidate.hiddenDoc, candidate.assessment);
      sanityChanges.push({
  ...
  writeCsv(paths.approvalBatch, [
    ...
    return selectedForApply
      ? { ...row, "approval decision": APPLY ? "approved in Sprint 12G" : "selected for approval dry-run", notes: APPLY ? "Sanity public approval applied" : "dry-run selection only" }
      : row;
  }));
  ...
  const staticRows = staticCandidates.map((row) => ({
    ...
    "content readiness": "monitoring only in Sprint 12G unless full route gates pass",
    ...
    notes: "Sprint 12G does not add static pages unless full gates pass."
  }));
  ...
  writeMd(paths.lawzana, `# Sprint 12G Lawzana Profile Verification
...
`);
  ...
  writeMd(paths.indexingPack, `# Sprint 12G GSC/Bing Indexing Pack
...
`);
  ...
  const result = {
    ...
    selected: selected.map((item) => ({
      ...
      title: item.hiddenDoc?.title || item.row.title,
  ...
  writeMd(paths.report, `# Sprint 12G Report
    ...
    JSON.stringify(
      {
        ...
        message: error?.message || "Sprint 12G helper failed",
        code: error?.code || error?.cause?.code || "",
        note: "Sanitized error output; request headers, tokens, and secrets are intentionally suppressed."
      },
      null,
      2
    )
