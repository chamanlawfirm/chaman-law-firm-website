...
const DRAFT_PREFIX = "drafts.chamanlawfirm.sprint10h.";
const PUBLIC_PREFIX = "chamanlawfirm-sprint10h-";
const SOURCE_NAME = "legacy-wordpress-backup-2026-06-28-sprint10h";

const shouldApply = process.argv.includes("--apply");
const selectionLimit = 40;
const draftLimit = 30;
const approvalLimit = 16;
...
const extractedImageRoot = "C:\\tmp\\chaman-sprint10h-images";

const top1000Path = "docs/SPRINT-9M-TOP-1000-LEGACY-BLOG-SELECTION.csv";
const nextConfigPath = "next.config.mjs";
const outDir = "docs/sprint10h";
const selectionCsvPath = "docs/SPRINT-10H-LEGACY-BLOG-BATCH-SELECTION.csv";
const imageMapPath = "docs/SPRINT-10H-LEGACY-IMAGE-RECOVERY-MAP.csv";
const hiddenReviewPath = "docs/SPRINT-10H-REMAINING-HIDDEN-DRAFT-REVIEW.csv";
const resultJsonPath = "docs/sprint10h/sprint10h-restoration-result.json";
...
async function sanityState(client) {
  const [approved, sprint10eDrafts, sprint10hDrafts] = await Promise.all([
    ...
    client.fetch('*[_type == "post" && _id in path("drafts.chamanlawfirm.sprint10e.*")] {_id,title,"slug":slug.current,lawFirmApproved,author->{name},mainImage{alt,asset->{_id}},seo{metaTitle,metaDescription,canonicalUrl},body}'),
    client.fetch('*[_type == "post" && _id in path("drafts.chamanlawfirm.sprint10h.*")] {_id,title,"slug":slug.current,lawFirmApproved}')
  ]);
  return { approved, sprint10eDrafts, sprint10hDrafts };
}
...
function reviewHiddenDrafts(drafts, redirectMap) {
  return drafts
    ...
    .map((draft) => {
      ...
      const errors = [];
      if (!Array.isArray(draft.body) || draft.body.length < 20 || text.length < 900) errors.push("body/formatting requires lawyer-editorial review");
      if (!draft.mainImage?.asset?._id) errors.push("article-specific image missing");
      if (!draft.mainImage?.alt) errors.push("alt text missing");
      if (!draft.seo?.metaTitle || !draft.seo?.metaDescription) errors.push("SEO metadata incomplete");
      if (disallowedPattern.test(`${draft.title} ${text}`)) errors.push("off-brand content signal");
      if (riskyFreePattern.test(`${draft.title} ${text}`)) errors.push("misleading free-service wording");
      const slug = draft.slug || "";
      return {
        title: draft.title,
        slug,
        "body completeness": Array.isArray(draft.body) ? `${draft.body.length} blocks` : "missing",
        "SEO title": draft.seo?.metaTitle ? "yes" : "no",
        "meta description": draft.seo?.metaDescription ? "yes" : "no",
        canonical: draft.seo?.canonicalUrl || "",
        "public author": draft.author?.name || "",
        "image status": draft.mainImage?.asset?._id ? "present" : "missing",
        "alt text": draft.mainImage?.alt || "",
        "legal safety": errors.length ? "blocked" : "passes automated checks",
        formatting: Array.isArray(draft.body) && draft.body.length >= 20 ? "acceptable" : "needs review",
        "internal links": text.includes("practice area") || text.includes("consultation") ? "present via CTA/body" : "needs review",
        "consultation CTA": text.includes("book a consultation") || text.includes("consultation") ? "present" : "needs review",
        "old URL mapping": `https://chamanlawfirm.com/${slug}/`,
        "current temporary redirect target": redirectMap.get(`/${slug}`) || "not currently mapped",
        "readiness for approval": errors.length ? "keep hidden" : "ready for manual approval consideration",
        notes: errors.join("; ")
      };
    });
}
...
async function main() {
  ...
  const [topRows, rankRows, nextConfigText] = await Promise.all([
    readCsv(top1000Path),
  ...
  const existingDraftSlugs = new Set([
    ...state.sprint10eDrafts.map((doc) => doc.slug).filter(Boolean),
    ...state.sprint10hDrafts.map((doc) => doc.slug).filter(Boolean)
  ]);

  const hiddenReviewRows = reviewHiddenDrafts(state.sprint10eDrafts, redirectMap);
  await fsp.writeFile(
    hiddenReviewPath,
  ...
  const rawCandidates = topRows
    ...
    .filter((row) => !approvedSlugs.has(row["old slug"]))
    .filter((row) => !existingDraftSlugs.has(row["old slug"]))
    .slice(0, 180)
    .map((row) => ({
      ...row,
  ...
  for (const [index, item] of selected.entries()) {
    ...
    const slug = row["old slug"];
    const imageAlt = cleanText(imageInfo.attachmentMeta?._wp_attachment_image_alt || imageInfo.attachment?.post_title || `${row["old title"] || post.post_title} legal guide | Chaman Law Firm`);
    ...
    selectionRows.push({
      ...
      "current production status": approvedSlugs.has(slug) ? "already public" : "selected for Sprint 10H recovery",
  ...
  const result = {
    ...
    remainingSprint10eHiddenDrafts: hiddenReviewRows.length,
