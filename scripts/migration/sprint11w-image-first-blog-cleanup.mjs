...
const APPROVAL_LIMIT = Number(process.env.SPRINT_11W_APPROVAL_LIMIT || 5);
const MIN_BODY_CHARS = 1400;
const MIN_BODY_BLOCKS = 8;
const EDITORIAL_APPROVAL_SLUGS = new Set(
  String(process.env.SPRINT_11W_APPROVAL_SLUGS || "")
    .split(",")
    .map((slug) => slug.trim())
    .filter(Boolean)
);
...
const paths = {
  ...
  buildStability: path.join("docs", "SPRINT-11W-LOCAL-BUILD-STABILITY-CHECK.md"),
  freshEvidence: path.join("docs", "SPRINT-11W-FRESH-GSC-BING-EVIDENCE-STATUS.md"),
  serviceMonitoring: path.join("docs", "SPRINT-11W-SERVICE-PAGE-AND-REDIRECT-MONITORING.csv"),
  redirectMonitoring: path.join("docs", "SPRINT-11W-EXISTING-REDIRECT-MONITORING.csv"),
  blogRecovery: path.join("docs", "SPRINT-11W-IMAGE-FIRST-HIDDEN-BLOG-CLEANUP-SELECTION.csv"),
  imageCtaMetadata: path.join("docs", "SPRINT-11W-IMAGE-ALT-CTA-METADATA-REPAIR.csv"),
  legalEditorial: path.join("docs", "SPRINT-11W-LEGAL-EDITORIAL-REDIRECT-CONFLICT-CLEANUP.csv"),
  approvalBatch: path.join("docs", "SPRINT-11W-CONTROLLED-BLOG-APPROVAL-BATCH.csv"),
  staticRecovery: path.join("docs", "SPRINT-11W-CONTINUED-SERVICE-PAGE-RECOVERY.csv"),
  redirectRescue: path.join("docs", "SPRINT-11W-REDIRECT-RESCUE-AND-404-REDUCTION.csv"),
  lawzana: path.join("docs", "SPRINT-11W-LAWZANA-PROFILE-VERIFICATION.md"),
  seoGeo: path.join("docs", "SPRINT-11W-SEO-AEO-GEO-ENHANCEMENT.csv"),
  indexingPack: path.join("docs", "SPRINT-11W-GSC-BING-INDEXING-PACK.md"),
  report: path.join("docs", "SPRINT-11W-REPORT.md"),
  result: path.join("docs", "SPRINT-11W-RESULT.json")
};
...
async function fetchManual(url) {
  try {
    const response = await fetch(url, {
      redirect: "manual",
      headers: {
        "user-agent": "ChamanLawFirm-Sprint11W-QA/1.0"
      }
    });
    return {
      ok: true,
...
function docCompleteness(doc, row, configuredRedirects) {
  ...
  if (EDITORIAL_APPROVAL_SLUGS.size && !EDITORIAL_APPROVAL_SLUGS.has(slug)) {
    blockers.push("not selected for Sprint 11W controlled editorial approval");
  }
...
function publicSafePostId(slug) {
  return `chamanlawfirm-sprint11w-${slug}`;
}
...
function currentRouteSetFromPracticeAreas() {
  ...
  // The data file interleaves practice-area and service-page slugs. For Sprint 11W,
...
async function main() {
  ...
  const manualApprovalRows = [...EDITORIAL_APPROVAL_SLUGS].map((slug, index) => ({
    priority: `11W-${index + 1}`,
    "old URL": absoluteUrl(`/${slug}/`),
    slug,
    title: titleFromSlug(slug),
    category: "Sprint 11W explicit low-risk hidden Sanity candidate"
  }));
  ...
  writeMd(
    ...
    `# Sprint 11W Local Build Stability Check
...
- Stability decision: safe to continue Sprint 11W, while continuing to monitor Sanity network fetch warnings during later builds.
...
`
  ...
  writeMd(
    paths.freshEvidence,
    `# Sprint 11W Fresh GSC/Bing Evidence Status
...
`
  ...
  writeCsv(paths.approvalBatch, [
    ...
    return selectedForApply
      ? { ...row, "approval decision": APPLY ? "approved in Sprint 11W" : "selected for approval dry-run", notes: APPLY ? "Sanity public approval applied" : "dry-run selection only" }
      : row;
  }));
  ...
  const staticRows = staticCandidates.map((row) => ({
    ...
    "content readiness": "monitoring only in Sprint 11W unless full route gates pass",
    ...
    notes: "Sprint 11W does not add static pages unless full gates pass."
  }));
  ...
  writeMd(paths.lawzana, `# Sprint 11W Lawzana Profile Verification
...
`);
  ...
  writeMd(paths.indexingPack, `# Sprint 11W GSC/Bing Indexing Pack
...
`);
  ...
  writeMd(paths.report, `# Sprint 11W Report
