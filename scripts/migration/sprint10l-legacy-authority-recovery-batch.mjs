...
const PUBLIC_PREFIX = "chamanlawfirm-sprint10l-";
const SOURCE_NAME = "legacy-wordpress-backup-2026-06-28-sprint10l";

const shouldApply = process.argv.includes("--apply");
const selectionLimit = 100;
const draftLimit = 100;
const approvalLimit = 40;
...
const extractedImageRoot = path.join(process.cwd(), ".tmp", "chaman-sprint10l-images");

const top1000Path = "docs/SPRINT-9M-TOP-1000-LEGACY-BLOG-SELECTION.csv";
const nextConfigPath = "next.config.mjs";
const outDir = "docs/sprint10l";
const selectionCsvPath = "docs/SPRINT-10L-100-BLOG-RECOVERY-SELECTION.csv";
const imageMapPath = "docs/SPRINT-10L-IMAGE-RECOVERY-AND-REFRESH-MAP.csv";
const hiddenReviewPath = "docs/SPRINT-10L-HIDDEN-RISK-CLEANUP.csv";
const resultJsonPath = "docs/sprint10l/sprint10l-restoration-result.json";
...
async function main() {
  ...
  for (const [index, item] of selected.entries()) {
    ...
    selectionRows.push({
      ...
      "current production status": approvedSlugs.has(slug) ? "already public" : "selected for Sprint 10L recovery",
    ...
    imageRows.push({
      ...
      "repeated image risk": "not detected in Sprint 10L recovered image evidence",
...
main().catch((error) => {
  console.error(`Sprint 10L pipeline failed: ${error?.message || String(error)}`);
  process.exit(1);
});
