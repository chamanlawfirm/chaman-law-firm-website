...
const APPROVAL_LIMIT = Number(process.env.SPRINT_11U_APPROVAL_LIMIT || 20);
...
const paths = {
  ...
  sprint11tBlog: path.join("docs", "SPRINT-11T-BLOG-REVIVAL-DEEP-REPAIR-BATCH.csv"),
  nextConfig: "next.config.mjs",
  freshEvidence: path.join("docs", "SPRINT-11U-FRESH-GSC-BING-EVIDENCE-STATUS.md"),
  serviceMonitoring: path.join("docs", "SPRINT-11U-SPRINT-11T-SERVICE-PAGE-MONITORING.csv"),
  redirectMonitoring: path.join("docs", "SPRINT-11U-SPRINT-11T-REDIRECT-MONITORING.csv"),
  blogRecovery: path.join("docs", "SPRINT-11U-HIGH-VALUE-BLOG-RECOVERY-BATCH.csv"),
  approvalBatch: path.join("docs", "SPRINT-11U-CONTROLLED-BLOG-APPROVAL-BATCH.csv"),
  staticRecovery: path.join("docs", "SPRINT-11U-CONTINUED-STATIC-SERVICE-RECOVERY.csv"),
  redirectRescue: path.join("docs", "SPRINT-11U-REDIRECT-RESCUE-AND-404-REDUCTION.csv"),
  lawzana: path.join("docs", "SPRINT-11U-LAWZANA-PROFILE-VERIFICATION.md"),
  seoGeo: path.join("docs", "SPRINT-11U-SEO-AEO-GEO-ENHANCEMENT.csv"),
  indexingPack: path.join("docs", "SPRINT-11U-GSC-BING-INDEXING-PACK.md"),
  report: path.join("docs", "SPRINT-11U-REPORT.md"),
  result: path.join("docs", "SPRINT-11U-RESULT.json")
};
...
async function fetchManual(url) {
  try {
    const response = await fetch(url, {
      redirect: "manual",
      headers: {
        "user-agent": "ChamanLawFirm-Sprint11U-QA/1.0"
      }
    });
    return {
      ok: true,
...
function docCompleteness(doc, row, configuredRedirects) {
  ...
  if (!doc?.mainImage?.alt || doc.mainImage.alt.trim().length < 12) blockers.push("alt text missing or weak");
  ...
  if (!EDITORIAL_APPROVAL_SLUGS.has(slug)) blockers.push("not selected for Sprint 11U controlled editorial approval");
...
function chooseHiddenDoc(docs) {
  const hidden = docs.filter((doc) => doc.lawFirmApproved !== true || doc._id.startsWith("drafts."));
...
function patchablePublicDoc(hiddenDoc, row, assessment) {
  ...
  const sourceId = hiddenDoc._id.startsWith("drafts.") ? hiddenDoc._id.replace(/^drafts\./, "") : hiddenDoc._id;
  ...
  return {
    ...
    seo: {
      ...
      ...(hiddenDoc.mainImage ? { openGraphImage: hiddenDoc.mainImage } : {})
    },
...
async function approveCandidate(row, hiddenDoc, assessment) {
  ...
  if (hiddenDoc._id.startsWith("drafts.")) {
...
function currentRouteSetFromPracticeAreas() {
  ...
  // The data file interleaves practice-area and service-page slugs. For Sprint 11U,
...
async function main() {
  ...
  const sprint11tBlogRows = readCsv(paths.sprint11tBlog).slice(0, 166);
  ...
  writeMd(
    paths.freshEvidence,
    `# Sprint 11U Fresh GSC/Bing Evidence Status
...
`
  ...
  for (const row of sprint11tBlogRows) {
    ...
    blogRecoveryRows.push({
      ...
      image: hiddenDoc?.mainImage?.asset?._id ? "image present" : "image missing",
      "alt text": hiddenDoc?.mainImage?.alt || "missing",
      ...
      category: (hiddenDoc?.categories || []).map((category) => category.title).join("; ") || row.category || "",
    ...
    approvalRows.push({
      ...
      reason,
      author: AUTHOR_NAME,
      "image status": hiddenDoc?.mainImage?.asset?._id ? "image present" : "image missing",
    ...
    seoRows.push({
      ...
      "image and alt text included": hiddenDoc?.mainImage?.asset?._id && hiddenDoc?.mainImage?.alt ? "yes" : "no",
      ...
      "topic cluster/internal-link relevance": (hiddenDoc?.categories || []).map((category) => category.title).join("; ") || "needs review",
  ...
  writeCsv(paths.approvalBatch, [
    ...
    return selectedForApply
      ? { ...row, "approval decision": APPLY ? "approved in Sprint 11U" : "selected for approval dry-run", notes: APPLY ? "Sanity public approval applied" : "dry-run selection only" }
      : row;
  }));
  ...
  const staticRows = staticCandidates.map((row) => ({
    ...
    "content readiness": "planning only in Sprint 11U unless full route gates pass",
    ...
    notes: "Sprint 11U does not add static pages unless full gates pass."
  }));
  ...
  writeMd(paths.lawzana, `# Sprint 11U Lawzana Profile Verification
...
`);
  ...
  writeMd(paths.indexingPack, `# Sprint 11U GSC/Bing Indexing Pack
...
`);
  ...
  writeMd(paths.report, `# Sprint 11U Report
