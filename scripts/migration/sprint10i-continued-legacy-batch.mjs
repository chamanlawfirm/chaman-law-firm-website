...
const DRAFT_PREFIX = "drafts.chamanlawfirm.sprint10i.";
const PUBLIC_PREFIX = "chamanlawfirm-sprint10i-";
const SOURCE_NAME = "legacy-wordpress-backup-2026-06-28-sprint10i";
...
const extractedImageRoot = "C:\\tmp\\chaman-sprint10i-images";

const top1000Path = "docs/SPRINT-9M-TOP-1000-LEGACY-BLOG-SELECTION.csv";
const nextConfigPath = "next.config.mjs";
const outDir = "docs/sprint10i";
const selectionCsvPath = "docs/SPRINT-10I-LEGACY-BLOG-BATCH-SELECTION.csv";
const imageMapPath = "docs/SPRINT-10I-LEGACY-IMAGE-RECOVERY-MAP.csv";
const hiddenReviewPath = "docs/SPRINT-10I-HIDDEN-DRAFT-CLEANUP.csv";
const resultJsonPath = "docs/sprint10i/sprint10i-restoration-result.json";
...
async function sanityState(client) {
  const [approved, hiddenDrafts] = await Promise.all([
    ...
    client.fetch(`*[_type == "post" && lawFirmApproved != true && (
      _id in path("drafts.chamanlawfirm.sprint10e.*") ||
      _id in path("drafts.chamanlawfirm.sprint10g.*") ||
      _id in path("drafts.chamanlawfirm.sprint10h.*") ||
  ...
  return drafts
    ...
    .map((draft) => {
      ...
      return {
        ...
        "duplicate/thin risk": bodyNeedsRecovery ? "thin/body completeness review needed" : "no obvious thin risk",
...
async function main() {
  ...
  const hiddenReviewRows = reviewHiddenDrafts(state.hiddenDrafts, redirectMap);
  await fsp.writeFile(
    hiddenReviewPath,
  ...
  for (const [index, item] of selected.entries()) {
    ...
    selectionRows.push({
      ...
      "current production status": approvedSlugs.has(slug) ? "already public" : "selected for Sprint 10I recovery",
