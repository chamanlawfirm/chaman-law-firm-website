...
const DRAFT_PREFIX = "drafts.chamanlawfirm.sprint10k.";
const PUBLIC_PREFIX = "chamanlawfirm-sprint10k-";
const SOURCE_NAME = "legacy-wordpress-backup-2026-06-28-sprint10k";
...
const extractedImageRoot = "C:\\tmp\\chaman-sprint10k-images";

const top1000Path = "docs/SPRINT-9M-TOP-1000-LEGACY-BLOG-SELECTION.csv";
const nextConfigPath = "next.config.mjs";
const outDir = "docs/sprint10k";
const selectionCsvPath = "docs/SPRINT-10K-LEGACY-BLOG-BATCH-SELECTION.csv";
const imageMapPath = "docs/SPRINT-10K-LEGACY-IMAGE-RECOVERY-MAP.csv";
const hiddenReviewPath = "docs/SPRINT-10K-HIDDEN-RISK-CLEANUP.csv";
const resultJsonPath = "docs/sprint10k/sprint10k-restoration-result.json";
...
const controlledApprovalSlugs = [
  ...
  "obtaining-a-c-of-o-in-ogun-state",
  ...
  "business-name-and-a-company-limited-by-shares",
...
async function main() {
  ...
  const [topRows, rankRows, nextConfigText, sprint10iResultText, sprint10jResultText] = await Promise.all([
    readCsv(top1000Path),
  ...
  const draftItems = controlledApprovalSlugs.length
    ? selected.filter((item) => controlledApprovalSlugs.includes(item.row["old slug"])).slice(0, draftLimit)
    : selected.slice(0, draftLimit);
  ...
  if (shouldApply) {
    ...
    await withRetry("create author", () =>
      client.createIfNotExists({ _id: AUTHOR_ID, _type: "author", name: AUTHOR_NAME, slug: { _type: "slug", current: "charles-chukwuma-nkwoka" } })
    );
  }
  ...
  for (const [index, item] of selected.entries()) {
    ...
    if (shouldApply && inDraftBatch && imageInfo.selectedEntry) {
    ...
    selectionRows.push({
      ...
      "current production status": approvedSlugs.has(slug) ? "already public" : "selected for Sprint 10K recovery",
  ...
  if (shouldApply) {
    ...
    for (const category of categories.values()) await withRetry(`create category ${category._id}`, () => client.createIfNotExists(category));

    for (const item of docs) {
      const draftDoc = { ...item.built.doc, lawFirmApproved: false };
      await withRetry(`create draft ${draftDoc._id}`, () => client.createOrReplace(draftDoc));
    }
    ...
    for (const item of approvedThisRun) {
      ...
      await withRetry(`create public post ${publicDoc._id}`, () => client.createOrReplace(publicDoc));
      await withRetry(`mark draft approved ${DRAFT_PREFIX}${slug}`, () => client.patch(`${DRAFT_PREFIX}${slug}`).set({ lawFirmApproved: true }).commit());
    }
  }
...
main().catch((error) => {
  console.error(`Sprint 10K pipeline failed: ${error?.message || String(error)}`);
  process.exit(1);
});
