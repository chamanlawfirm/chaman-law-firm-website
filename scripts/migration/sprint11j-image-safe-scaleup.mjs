...
const PUBLIC_PREFIX = "chamanlawfirm-sprint11j-";
const PRIORITY_REVIEW_LIMIT = 100;
const BLOG_PREP_LIMIT = 50;
...
const paths = {
  ...
  freshEvidence: path.join("docs", "SPRINT-11J-FRESH-SEARCH-EVIDENCE-STATUS.md"),
  priorityReview: path.join("docs", "SPRINT-11J-PRIORITY-100-LEGACY-CANDIDATE-REVIEW.csv"),
  authorAudit: path.join("docs", "SPRINT-11J-PUBLIC-SAFE-AUTHOR-AUDIT.csv"),
  imageLibrary: path.join("docs", "SPRINT-11J-APPROVED-LEGAL-IMAGE-LIBRARY.csv"),
  imageRecovery: path.join("docs", "SPRINT-11J-IMAGE-SAFE-BLOG-RECOVERY-BATCH.csv"),
  legalReview: path.join("docs", "SPRINT-11J-LAWYER-CURRENT-LAW-REVIEW.csv"),
  metadataCompletion: path.join("docs", "SPRINT-11J-METADATA-INTERNAL-LINKING-AEO-GEO-COMPLETION.csv"),
  approvalBatch: path.join("docs", "SPRINT-11J-BLOG-APPROVAL-BATCH.csv"),
  staticContinuation: path.join("docs", "SPRINT-11J-STATIC-SERVICE-AUTHORITY-CONTINUATION.csv"),
  redirectBatch: path.join("docs", "SPRINT-11J-REDIRECT-ACTIVATION-BATCH.csv"),
  report: path.join("docs", "SPRINT-11J-IMAGE-SAFE-RECOVERY-REPORT.md"),
  resultJson: path.join("docs", "SPRINT-11J-RESULT.json")
};
...
function candidateBlockers({ row, doc, selected, image, textRisks, legalRisk, duplicateRisk }) {
  ...
  if (!selected) blockers.push("not selected by Sprint 11J image-safe public gate");
...
async function main() {
  ...
  const sortedMasterRows = masterRows
    .filter((row) => row["old URL"])
    .sort((a, b) => Number(b["SEO priority score"] || 0) - Number(a["SEO priority score"] || 0));
  const priorityRows = sortedMasterRows.slice(0, PRIORITY_REVIEW_LIMIT);
  const blogSeedRows = readCsv(paths.blogBatch).map((row) => ({
    ...row,
    slug: row.slug || slugFromUrl(row["old URL"])
  }));
  const blogBySlug = new Map();
  const addBlogRow = (row) => {
    const slug = row.slug || slugFromUrl(row["recommended final URL"] || row["old URL"]);
    if (!slug || blogBySlug.has(slug)) return;
    blogBySlug.set(slug, { ...row, slug });
  };
  for (const row of sortedMasterRows) {
    const finalUrl = row["recommended final URL"] || "";
    const likelyType = `${row["likely page type"] || ""} ${row["old content type"] || ""}`.toLowerCase();
    if (finalUrl.includes("/resources/blog/") || likelyType.includes("blog") || likelyType.includes("post")) {
      addBlogRow({
        "old URL": row["old URL"],
        slug: slugFromUrl(finalUrl || row["old URL"]),
        "old title": row["old title"],
        "improved title": row["old title"],
        "GSC clicks": row["GSC clicks"],
        "GSC impressions": row["GSC impressions"],
        "SEO title status": row["old meta title"] ? "present" : "missing",
        "meta description status": row["old meta description"] ? "present" : "missing",
        "category/practice relationship": row["likely page type"],
        "legal risk": row["legal risk"],
        "duplicate/cannibalization risk": row["duplicate/cannibalization risk"],
        "source inventory note": row.notes || ""
      });
    }
  }
  blogSeedRows.forEach(addBlogRow);
  let blogRows = [...blogBySlug.values()];
  ...
  blogRows = blogRows
    .filter((row) => !(approvedBySlug.get(row.slug) || []).length)
    .slice(0, BLOG_PREP_LIMIT);
  ...
  const priorityReviewRows = priorityRows.map((row, index) => {
    const oldPath = normalizePath(row["old URL"]);
    const finalPath = normalizePath(row["recommended final URL"] || row["old URL"]);
    const slug = slugFromUrl(finalPath);
    const docs = refreshedBySlug.get(slug) || [];
    const publicDocs = approvedBySlug.get(slug) || [];
    const hiddenDocs = docs.filter((doc) => doc.lawFirmApproved !== true || doc._id.startsWith("drafts."));
    ...
    return {
      ...
      "GSC clicks": row["GSC clicks"],
      "GSC impressions": row["GSC impressions"],
      "SEO priority score": row["SEO priority score"],
      "current redirect target": configuredRedirect ? absoluteUrl(configuredRedirect) : row["current redirect target"] || "",
      "recommended final URL": absoluteUrl(finalPath),
      "public Sanity status": publicDocs.length ? "approved public" : "not public",
      "hidden/unapproved Sanity records": hiddenDocs.length,
      "image status": row["image status"] || row["old image availability"] || "",
      "legal risk": row["legal risk"],
      "duplicate/cannibalization risk": row["duplicate/cannibalization risk"],
      "Sprint 11J action": publicDocs.length
        ? "verify live/redirect only"
        : finalPath.includes("/resources/blog/")
          ? "prepare as blog candidate if gates pass"
          : "static/service authority planning",
      notes: row.notes || ""
    };
  });
  ...
  for (const row of blogRows) {
    ...
    const oldPath = normalizePath(row["old URL"]);
    const configuredRedirect = redirects.get(oldPath);
    const redirectConflict = configuredRedirect && configuredRedirect !== targetPath;
    const duplicateRisk = redirectConflict
      ? `high - old URL already redirects to ${configuredRedirect}`
      : duplicateRiskFor(row.slug, row["duplicate/cannibalization risk"]);
    ...
    const prelimRepairable =
      sourceDoc &&
      !alreadyPublic &&
      text.length >= MIN_BODY_CHARACTERS &&
      !legalRisk.startsWith("high") &&
      !legalRisk.startsWith("medium") &&
    ...
    const preliminaryBlockers = candidateReady
      ? candidateBlockers({ row, doc: sourceDoc, selected: true, image, textRisks, legalRisk, duplicateRisk })
      : ["not preliminary-ready"];
    if (candidateReady && preliminaryBlockers.length === 0 && selectedRows.length < APPROVAL_LIMIT) selectedRows.push(row.slug);
    ...
    legalRows.push({
      ...
      slug: row.slug,
      title,
      "complete body": text.length >= MIN_BODY_CHARACTERS ? "yes" : "no",
      "short/thin body risk": text.length >= MIN_BODY_CHARACTERS ? "no" : "yes",
    ...
    metadataRows.push({
      ...
      slug: row.slug,
      title,
      "SEO title": seoFor(row.slug, title, sourceDoc).title,
      "meta description": seoFor(row.slug, title, sourceDoc).description,
      canonical: targetUrl,
      "public author": AUTHOR_NAME,
      "answer block": canApprove ? "added to approved public document" : "recommended before approval",
      "legal education disclaimer": canApprove ? "added or preserved" : "recommended before approval",
      "consultation CTA": canApprove ? "present or added" : hasConsultationSignal(text) ? "present in source" : "needed before approval",
      "internal links": hasHref(sourceDoc?.body || []) ? "present" : canApprove ? "consultation/contact links added" : "needed before approval",
      "practice/category relationship": categories.join("; ") || row["category/practice relationship"] || "missing",
      "AEO/GEO status": canApprove ? "answer block and Chaman Law Firm entity framing added" : "pending",
      "sitemap expectation": canApprove ? "include after Sanity approval and sitemap revalidation" : "exclude while hidden",
      notes: blockers.join("; ")
    });
    ...
    approvalRows.push({
      ...
      slug: row.slug,
      title,
      "selected for Sprint 11J": selected ? "yes" : "no",
      "body status": text.length >= MIN_BODY_CHARACTERS ? "present" : text ? "short/review needed" : "missing",
  ...
  writeOutputFiles({
    ...
    result: {
      ...
      imagePatchResults,
      priorityCandidatesReviewed: priorityReviewRows.length,
...
function writeOutputFiles(data) {
  const freshMd = `# Sprint 11J Fresh Search Evidence Status
...
`;
  ...
  writeCsv(paths.priorityReview, [
    ...
    "public Sanity status",
    "hidden/unapproved Sanity records",
    "image status",
    "legal risk",
    "duplicate/cannibalization risk",
    "Sprint 11J action",
    "notes"
  ], data.priorityReviewRows);
  ...
  writeCsv(paths.metadataCompletion, [
    ...
    "consultation CTA",
    "internal links",
    "practice/category relationship",
    "AEO/GEO status",
    "sitemap expectation",
    "notes"
  ], data.metadataRows);
  ...
  fs.writeFileSync(paths.report, `# Sprint 11J Image-Safe Legacy Blog Recovery and Authority Restoration Report
...
- Priority legacy candidates reviewed: ${data.result.priorityCandidatesReviewed}
- Blog candidates reviewed: ${data.result.blogCandidatesReviewed}
- Blog articles approved in Sprint 11J: ${data.result.approvedCount}
