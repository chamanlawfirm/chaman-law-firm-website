...
const APPROVAL_LIMIT = Number(process.env.SPRINT_11X_APPROVAL_LIMIT || 5);
const BULK_SCAN_LIMIT = Number(process.env.SPRINT_11X_BULK_SCAN_LIMIT || 500);
const MIN_BODY_CHARS = 1400;
const MIN_BODY_BLOCKS = 8;
const EDITORIAL_APPROVAL_SLUGS = new Set(
  String(process.env.SPRINT_11X_APPROVAL_SLUGS || "")
    .split(",")
    .map((slug) => slug.trim())
    .filter(Boolean)
);
...
const paths = {
  ...
  buildStability: path.join("docs", "SPRINT-11X-LOCAL-BUILD-STABILITY-CHECK.md"),
  freshEvidence: path.join("docs", "SPRINT-11X-FRESH-GSC-BING-EVIDENCE-STATUS.md"),
  serviceMonitoring: path.join("docs", "SPRINT-11X-SERVICE-PAGE-AND-REDIRECT-MONITORING.csv"),
  redirectMonitoring: path.join("docs", "SPRINT-11X-EXISTING-REDIRECT-MONITORING.csv"),
  bulkImport: path.join("docs", "SPRINT-11X-BULK-LEGACY-IMPORT-TO-DRAFT.csv"),
  blogRecovery: path.join("docs", "SPRINT-11X-FAST-PUBLISH-CANDIDATE-SELECTION.csv"),
  imageCtaMetadata: path.join("docs", "SPRINT-11X-BLOG-STANDARDIZATION-AND-ENHANCEMENT.csv"),
  legalEditorial: path.join("docs", "SPRINT-11X-LEGAL-EDITORIAL-REDIRECT-CONFLICT-CLEANUP.csv"),
  approvalBatch: path.join("docs", "SPRINT-11X-CONTROLLED-PUBLISHING-BATCH.csv"),
  staticRecovery: path.join("docs", "SPRINT-11X-STATIC-PAGE-RECOVERY-AND-ENHANCEMENT.csv"),
  redirectRescue: path.join("docs", "SPRINT-11X-REDIRECT-RESCUE-AND-404-REDUCTION.csv"),
  lawzana: path.join("docs", "SPRINT-11X-LAWZANA-PROFILE-VERIFICATION.md"),
  seoGeo: path.join("docs", "SPRINT-11X-SEO-AEO-GEO-ENHANCEMENT.csv"),
  indexingPack: path.join("docs", "SPRINT-11X-GSC-BING-INDEXING-PACK.md"),
  report: path.join("docs", "SPRINT-11X-REPORT.md"),
  result: path.join("docs", "SPRINT-11X-RESULT.json")
};
...
async function fetchManual(url) {
  try {
    const response = await fetch(url, {
      redirect: "manual",
      headers: {
        "user-agent": "ChamanLawFirm-sprint11x-QA/1.0"
      }
    });
    return {
      ok: true,
...
function docCompleteness(doc, row, configuredRedirects) {
  ...
  if (EDITORIAL_APPROVAL_SLUGS.size && !EDITORIAL_APPROVAL_SLUGS.has(slug)) {
    blockers.push("not selected for Sprint 11X controlled editorial approval");
  }
...
function publicSafePostId(slug) {
  return `chamanlawfirm-sprint11x-${slug}`;
}
...
function currentRouteSetFromPracticeAreas() {
  ...
  // The data file interleaves practice-area and service-page slugs. For Sprint 11X,
...
async function main() {
  ...
  const hiddenSourceRows = hiddenSourceDocs.map((doc, index) => ({
    ...
    slug: doc.slug,
    title: TITLE_OVERRIDES.get(doc.slug) || doc.title || titleFromSlug(doc.slug),
  ...
  const manualApprovalRows = [...EDITORIAL_APPROVAL_SLUGS].map((slug, index) => ({
    priority: `11X-${index + 1}`,
    "old URL": absoluteUrl(`/${slug}/`),
    slug,
    title: titleFromSlug(slug),
    category: "Sprint 11X explicit low-risk hidden Sanity candidate"
  }));
...
`
  ...
  writeMd(
    paths.freshEvidence,
    `# Sprint 11X Fresh GSC/Bing Evidence Status
...
`
  ...
  const bulkRows = hiddenSourceDocs.map((doc, index) => {
    ...
    const risks = classifyRisk(doc.title || slug, text);
    const existingRedirect = configuredRedirects.get(pathOnly(oldUrl)) || configuredRedirects.get(`${pathOnly(oldUrl)}/`) || "";
    const imagePresent = hasImageRef(doc.mainImage);
    const altPresent = Boolean(doc.mainImage?.alt && doc.mainImage.alt.trim().length >= 12);
    const metadataPresent = Boolean(doc.seo?.metaTitle && doc.seo?.metaDescription && doc.seo?.canonicalUrl);
  ...
  for (const row of sprint11tBlogRows) {
    ...
    const reason = alreadyPublic
      ? publicRefreshReady ? "public canonical article exists; explicit Sprint 11X title/metadata refresh selected" : "public canonical article already exists"
      : approvalReady
  ...
  writeCsv(paths.approvalBatch, [
    ...
    return selectedForApply
      ? { ...row, "approval decision": APPLY ? "approved/refreshed in Sprint 11X" : "selected for approval/refresh dry-run", notes: APPLY ? "Sanity public approval or metadata refresh applied" : "dry-run selection only" }
      : row;
  }));
  ...
  const staticRows = staticCandidates.map((row) => ({
    ...
    "content readiness": "monitoring only in Sprint 11X unless full route gates pass",
    ...
    notes: "Sprint 11X does not add static pages unless full gates pass."
  }));
  ...
  writeMd(paths.lawzana, `# Sprint 11X Lawzana Profile Verification
...
`);
  ...
  writeMd(paths.indexingPack, `# Sprint 11X GSC/Bing Indexing Pack
...
`);
  ...
  writeMd(paths.report, `# Sprint 11X Report
