...
const PUBLIC_PREFIX = "chamanlawfirm-sprint10q-";
const SOURCE_NAME = "legacy-wordpress-backup-2026-06-28-sprint10q";

const shouldApply = process.argv.includes("--apply");
const selectionLimit = 100;
const draftLimit = 100;
const approvalLimit = 50;
...
const extractedImageRoot = path.join(process.cwd(), ".tmp", "chaman-sprint10q-images");

const top1000Path = "docs/SPRINT-9M-TOP-1000-LEGACY-BLOG-SELECTION.csv";
const nextConfigPath = "next.config.mjs";
const outDir = "docs/sprint10q";
const selectionCsvPath = "docs/SPRINT-10Q-100-BLOG-RECOVERY-SELECTION.csv";
const imageMapPath = "docs/SPRINT-10Q-IMAGE-RECOVERY-AND-REFRESH-MAP.csv";
const hiddenReviewPath = "docs/SPRINT-10Q-HIDDEN-RISK-CLEANUP.csv";
const resultJsonPath = "docs/sprint10q/sprint10q-restoration-result.json";
...
async function main() {
  ...
  for (const [index, item] of selected.entries()) {
    ...
    selectionRows.push({
      ...
      "current production status": approvedSlugs.has(slug) ? "already public" : "selected for Sprint 10O recovery",
    ...
    imageRows.push({
      ...
      "repeated image risk": "not detected in Sprint 10O recovered image evidence",
...
main().catch((error) => {
  console.error(`Sprint 10O pipeline failed: ${error?.message || String(error)}`);
  process.exit(1);
});
