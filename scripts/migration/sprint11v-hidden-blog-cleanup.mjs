...
const APPROVAL_LIMIT = Number(process.env.SPRINT_11V_APPROVAL_LIMIT || 10);
...
const paths = {
  ...
  freshEvidence: path.join("docs", "SPRINT-11V-FRESH-GSC-BING-EVIDENCE-STATUS.md"),
  serviceMonitoring: path.join("docs", "SPRINT-11V-SERVICE-PAGE-MONITORING.csv"),
  redirectMonitoring: path.join("docs", "SPRINT-11V-EXISTING-REDIRECT-MONITORING.csv"),
  blogRecovery: path.join("docs", "SPRINT-11V-HIDDEN-BLOG-CLEANUP-SELECTION.csv"),
  imageCtaMetadata: path.join("docs", "SPRINT-11V-IMAGE-CTA-METADATA-REPAIR.csv"),
  legalEditorial: path.join("docs", "SPRINT-11V-LEGAL-EDITORIAL-CLEANUP.csv"),
  approvalBatch: path.join("docs", "SPRINT-11V-CONTROLLED-BLOG-APPROVAL-BATCH.csv"),
  staticRecovery: path.join("docs", "SPRINT-11V-CONTINUED-SERVICE-PAGE-RECOVERY.csv"),
  redirectRescue: path.join("docs", "SPRINT-11V-REDIRECT-RESCUE-AND-404-REDUCTION.csv"),
  lawzana: path.join("docs", "SPRINT-11V-LAWZANA-PROFILE-VERIFICATION.md"),
  seoGeo: path.join("docs", "SPRINT-11V-SEO-AEO-GEO-ENHANCEMENT.csv"),
  indexingPack: path.join("docs", "SPRINT-11V-GSC-BING-INDEXING-PACK.md"),
  report: path.join("docs", "SPRINT-11V-REPORT.md"),
  result: path.join("docs", "SPRINT-11V-RESULT.json")
};
...
async function fetchManual(url) {
  try {
    const response = await fetch(url, {
      redirect: "manual",
      headers: {
        "user-agent": "ChamanLawFirm-Sprint11V-QA/1.0"
      }
    });
    return {
      ok: true,
...
function docCompleteness(doc, row, configuredRedirects) {
  ...
  if (EDITORIAL_APPROVAL_SLUGS.size && !EDITORIAL_APPROVAL_SLUGS.has(slug)) {
    blockers.push("not selected for Sprint 11V controlled editorial approval");
  }
...
function patchablePublicDoc(hiddenDoc, row, assessment) {
  ...
  const mainImage = assessment.imageCandidate || hiddenDoc.mainImage;
  return {
    ...hiddenDoc,
    _id: sourceId,
    _type: "post",
...
function currentRouteSetFromPracticeAreas() {
  ...
  // The data file interleaves practice-area and service-page slugs. For Sprint 11V,
...
async function main() {
  ...
  writeMd(
    paths.freshEvidence,
    `# Sprint 11V Fresh GSC/Bing Evidence Status
...
`
  ...
  writeCsv(paths.approvalBatch, [
    ...
    return selectedForApply
      ? { ...row, "approval decision": APPLY ? "approved in Sprint 11V" : "selected for approval dry-run", notes: APPLY ? "Sanity public approval applied" : "dry-run selection only" }
      : row;
  }));
  ...
  const staticRows = staticCandidates.map((row) => ({
    ...
    "content readiness": "monitoring only in Sprint 11V unless full route gates pass",
    ...
    notes: "Sprint 11V does not add static pages unless full gates pass."
  }));
  ...
  writeMd(paths.lawzana, `# Sprint 11V Lawzana Profile Verification
...
`);
  ...
  writeMd(paths.indexingPack, `# Sprint 11V GSC/Bing Indexing Pack
...
`);
  ...
  writeMd(paths.report, `# Sprint 11V Report
