...
const APPROVAL_LIMIT = Number(process.env.SPRINT_11Z_APPROVAL_LIMIT || 40);
const BULK_SCAN_LIMIT = Number(process.env.SPRINT_11Z_BULK_SCAN_LIMIT || 1000);
const HIDDEN_REPAIR_LIMIT = Number(process.env.SPRINT_11Z_HIDDEN_REPAIR_LIMIT || 600);
const BATCH_SIZE = Number(argValue("--batch-size", process.env.SPRINT_11Z_BATCH_SIZE || "25"));
const OFFSET_OVERRIDE = argValue("--offset", process.env.SPRINT_11Z_OFFSET || "");
const MIN_BODY_CHARS = 1400;
const MIN_BODY_BLOCKS = 8;
const EDITORIAL_APPROVAL_SLUGS = new Set(
  String(process.env.SPRINT_11Z_APPROVAL_SLUGS || "")
    .split(",")
    .map((slug) => slug.trim())
    .filter(Boolean)
);
...
const paths = {
  ...
  buildStability: path.join("docs", "SPRINT-11Z-LOCAL-BUILD-STABILITY-CHECK.md"),
  freshEvidence: path.join("docs", "SPRINT-11Z-FRESH-GSC-BING-EVIDENCE-STATUS.md"),
  serviceMonitoring: path.join("docs", "SPRINT-11Z-SERVICE-PAGE-AND-REDIRECT-MONITORING.csv"),
  redirectMonitoring: path.join("docs", "SPRINT-11Z-EXISTING-REDIRECT-MONITORING.csv"),
  bulkImport: path.join("docs", "SPRINT-11Z-BULK-HIDDEN-SOURCE-REPAIR.csv"),
  blogRecovery: path.join("docs", "SPRINT-11Z-FAST-PUBLISH-CANDIDATE-SELECTION.csv"),
  imageCtaMetadata: path.join("docs", "SPRINT-11Z-BLOG-STANDARDIZATION-AND-ENHANCEMENT.csv"),
  legalEditorial: path.join("docs", "SPRINT-11Z-LEGAL-EDITORIAL-CLEARANCE.csv"),
  approvalBatch: path.join("docs", "SPRINT-11Z-CONTROLLED-BLOG-PUBLICATION.csv"),
  staticRecovery: path.join("docs", "SPRINT-11Z-STATIC-SERVICE-RECOVERY.csv"),
  redirectRescue: path.join("docs", "SPRINT-11Z-REDIRECT-RESCUE.csv"),
  lawzana: path.join("docs", "SPRINT-11Z-LAWZANA-PROFILE-VERIFICATION.md"),
  seoGeo: path.join("docs", "SPRINT-11Z-SEO-AEO-GEO-ENHANCEMENT.csv"),
  indexingPack: path.join("docs", "SPRINT-11Z-GSC-BING-INDEXING-PACK.md"),
  report: path.join("docs", "SPRINT-11Z-REPORT.md"),
  result: path.join("docs", "SPRINT-11Z-RESULT.json"),
  checkpoint: path.join("docs", "SPRINT-11Z-HIDDEN-REPAIR-CHECKPOINT.json")
};
...
function loadCheckpoint() {
  if (RESET_CHECKPOINT) return initialCheckpoint();
  return readJson(paths.checkpoint, initialCheckpoint());
}
...
async function fetchManual(url) {
  try {
    const response = await fetch(url, {
      redirect: "manual",
      headers: {
        "user-agent": "ChamanLawFirm-sprint11z-QA/1.0"
      }
    });
    return {
      ok: true,
...
function docCompleteness(doc, row, configuredRedirects) {
  ...
  if (EDITORIAL_APPROVAL_SLUGS.size && !EDITORIAL_APPROVAL_SLUGS.has(slug)) {
    blockers.push("not selected for Sprint 11Z controlled editorial approval");
  }
...
function publicSafePostId(slug) {
  return `chamanlawfirm-sprint11z-${slug}`;
}
...
function currentRouteSetFromPracticeAreas() {
  ...
  // The data file interleaves practice-area and service-page slugs. For Sprint 11Z,
...
async function main() {
  ...
  const allHiddenRepairTargets = hiddenSourceDocs
    .filter((doc) => doc.slug && doc.lawFirmApproved !== true)
    .slice(0, HIDDEN_REPAIR_LIMIT);
  const checkpoint = loadCheckpoint();
  const resolvedOffset = OFFSET_OVERRIDE !== "" ? Number(OFFSET_OVERRIDE) : Number(checkpoint.nextOffset || 0);
  const batchOffset = Number.isFinite(resolvedOffset) && resolvedOffset >= 0 ? resolvedOffset : 0;
  const effectiveBatchSize = Number.isFinite(BATCH_SIZE) && BATCH_SIZE > 0 ? BATCH_SIZE : 25;
  const hiddenRepairTargets = allHiddenRepairTargets.slice(batchOffset, batchOffset + effectiveBatchSize);
  const hiddenRepairChanges = [];
  if (APPLY) {
    const batchResult = {
      batchNumber: checkpoint.completedBatches.length + 1,
      startOffset: batchOffset,
      batchSize: effectiveBatchSize,
      attempted: hiddenRepairTargets.length,
      succeeded: 0,
      skippedOrIdempotent: 0,
      failed: 0,
    ...
    for (const doc of hiddenRepairTargets) {
      try {
        ...
        const hiddenRepairApplied = operations.length > 0;
        ...
        if (hiddenRepairApplied) {
          batchResult.succeeded += 1;
          checkpoint.successfulIds.push({ id: doc._id, slug: doc.slug, at: new Date().toISOString() });
        } else {
          batchResult.skippedOrIdempotent += 1;
        }
      } catch (error) {
        const failure = {
          id: doc._id,
        ...
        hiddenRepairChanges.push({
          ...
          slug: doc.slug,
          operations: [],
          hiddenRepairApplied: false,
          approvalReadiness: "repair failed; keep hidden",
          error: failure.reason
        });
        checkpoint.failed.push(failure);
        batchResult.failed += 1;
      }
    }
    batchResult.completedAt = new Date().toISOString();
    checkpoint.nextOffset = Math.min(batchOffset + hiddenRepairTargets.length, allHiddenRepairTargets.length);
    checkpoint.completedBatches.push(batchResult);
  ...
  const manualApprovalRows = [...EDITORIAL_APPROVAL_SLUGS].map((slug, index) => ({
    priority: `11Z-${index + 1}`,
    "old URL": absoluteUrl(`/${slug}/`),
    slug,
    title: titleFromSlug(slug),
    category: "Sprint 11Z explicit low-risk hidden Sanity candidate"
  }));
...
- Stability decision: safe to continue Sprint 11Z, while continuing to monitor Sanity network fetch warnings during later builds.
...
`
  ...
  for (const row of sprint11tBlogRows) {
    ...
    const reason = alreadyPublic
      ? publicRefreshReady ? "public canonical article exists; explicit Sprint 11Z title/metadata refresh selected" : "public canonical article already exists"
      : approvalReady
  ...
  writeCsv(paths.approvalBatch, [
    ...
    return selectedForApply
        ? { ...row, "approval decision": PUBLISH ? "approved/refreshed in Sprint 11Z" : "selected for approval/refresh dry-run", notes: PUBLISH ? "Sanity public approval or metadata refresh applied" : "dry-run selection only" }
      : row;
  }));
  ...
  const staticRows = staticCandidates.map((row) => ({
    ...
    "content readiness": "monitoring only in Sprint 11Z unless full route gates pass",
    ...
    notes: "Sprint 11Z does not add static pages unless full gates pass."
  }));
  ...
  writeMd(paths.lawzana, `# Sprint 11Z Lawzana Profile Verification
...
`);
  ...
  writeMd(paths.indexingPack, `# Sprint 11Z GSC/Bing Indexing Pack
...
`);
  ...
  const result = {
    sprint: "11Z",
    mode: APPLY ? "repair-apply" : PUBLISH ? "publish-apply" : "dry-run",
    ...
    hiddenSourceRepairTargets: allHiddenRepairTargets.length,
    hiddenSourceRepairBatchOffset: batchOffset,
    hiddenSourceRepairBatchSize: effectiveBatchSize,
    hiddenSourceRepairBatchAttempted: hiddenRepairTargets.length,
    hiddenSourceRepairNextOffset: APPLY ? checkpoint.nextOffset : batchOffset + hiddenRepairTargets.length,
    hiddenSourceRepairCheckpointBatches: checkpoint.completedBatches.length,
    hiddenSourceRepairCheckpointSucceeded: checkpoint.successfulIds.length,
    hiddenSourceRepairCheckpointFailed: checkpoint.failed.length,
    ...
    approvedInSanity: PUBLISH ? sanityChanges.length : 0,
    liveApprovedPassed: PUBLISH ? liveApproved.filter((item) => item.live?.status === 200).length : 0,
  ...
  writeMd(paths.report, `# Sprint 11Z Report
